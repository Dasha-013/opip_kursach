...
shared_ptr<EmployeeUser> LoginSystem::signUp() {
    ...
    loginToSalt[login] = salt;
    loginToHashedPassword[login] = hash;
    loginToRole[login] = "viewer";
    string path = createUserFile(hash);
    writeData();
    cout << "Регистрация завершена!\n";
    return make_shared<EmployeeUser>(path, login, "viewer");
}
...
Base* LoginSystem::run()
{
    ...
    do {
        ...
        std::cout << "[0] Выход\n";
        getInRange("Ваш выбор: ", authChoice, 0, 2);
    ...
    while (true) {
        ...
        switch (choice) {
        case 1:
            if (role == "admin") return new AdminEmployeeBase(user);
            if (role == "manager") return new UserPayrollBase(user);
