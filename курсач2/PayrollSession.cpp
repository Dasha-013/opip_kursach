...
PayrollSession::PayrollSession() : sessionID(++sessionCounter) {}
PayrollSession::PayrollSession(const std::string& period, SalaryDateTime date)
    : sessionID(++sessionCounter), periodLabel(period), sessionDate(date) {}
...
void PayrollSession::generateSlips(const std::vector<std::shared_ptr<EmployeeUser>>& employees, double baseSalary) {
    salarySlips.clear();
    for (const auto& user : employees) {
        auto emp = user->getEmployee();
        if (!emp) continue;

        auto slip = std::make_shared<SalarySlip>(baseSalary, "Оклад", emp);
